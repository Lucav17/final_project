...
as.character.soql <- function(soql_list) {
  params <- '?'
  
  for(param_key in names(soql_list$clauses)) {
    if(params != '?') {
      params <- paste0(params, '&')
    }
    
    param_value <- as.character(soql_list$clauses[[param_key]])
  ...
  for(param_key in names(soql_list$simple_filters)) {
    if(params != '?') {
      params <- paste0(params, '&')
    }
    
    param_value <- as.character(soql_list$simple_filters[[param_key]])
  ...
  if(params == '?') {
    return(soql_list$endpoint)
  } else {
    # Null resolves to ''
    return(paste0(soql_list$endpoint, params))
  }
}

print.soql <- function(soql_list) {
  result <- as.character(soql_list)
  if(!is.null(result)) {
    cat(result)
  }
}
...
soql_simple_filter <- function(soql_list, column, value) {
  ...
  soql_list$simple_filters[[column]] <- value
